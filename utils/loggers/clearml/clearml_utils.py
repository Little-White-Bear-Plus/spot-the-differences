...
class ClearmlLogger:
    ...
    def __init__(self, opt, hyp):
        ...
        # Keep tracked of amount of logged data to enforce a limit
        self.current_epoch_logged_images = set()
        # Maximum number of data to log to clearML per epoch
        self.max_imgs_to_log_per_epoch = 16
        # Get the interval of epochs when bounding box data should be logged
        ...
        if self.clearml:
            ...
            # Make sure the code is easily remotely runnable by setting the docker data to use by the remote agent
    ...
    def log_debug_samples(self, files, title='Debug Samples'):
        """
        Log files (data) as debug samples in the ClearML task.

        arguments:
        files (List(PosixPath)) a list of file paths in PosixPath format
        title (str) A title that groups together data with the same values
        """
        for f in files:
            if f.exists():
    ...
    def log_image_with_boxes(self, image_path, boxes, class_names, image, conf_threshold=0.25):
        """
        Draw the bounding boxes on a single data and report the result as a ClearML debug sample.

        arguments:
        image_path (PosixPath) the path the original data file
        boxes (list): list of scaled predictions in the format - [xmin, ymin, xmax, ymax, confidence, class]
        class_names (dict): dict containing mapping of class int to class name
        data (Tensor): A torch tensor containing the actual data data
        """
