...
def export_onnx(model, im, file, opset, dynamic, simplify, prefix=colorstr('ONNX:')):
    ...
    if dynamic:
        dynamic = {'data': {0: 'batch', 2: 'height', 3: 'width'}}  # shape(1,3,640,640)
...
def export_coreml(model, im, file, int8, half, nms, prefix=colorstr('CoreML:')):
    ...
    ct_model = ct.convert(ts, inputs=[ct.ImageType('data', shape=im.shape, scale=1 / 255, bias=[0, 0, 0])])
...
def pipeline_coreml(model, im, file, names, y, prefix=colorstr('CoreML Pipeline:')):
    ...
    # flexible_shape_utils.add_enumerated_image_sizes(spec, feature_name='data', sizes=s)
    # r = flexible_shape_utils.NeuralNetworkImageSizeRange()  # shape ranges
    # r.add_height_range((192, 640))
    # r.add_width_range((192, 640))
    # flexible_shape_utils.update_image_size_range(spec, feature_name='data', size_range=r)
    ...
    pipeline = ct.models.pipeline.Pipeline(input_features=[('data', ct.models.datatypes.Array(3, ny, nx)),
    ...
    model.input_description['data'] = 'Input data'
    model.input_description['iouThreshold'] = f'(optional) IOU Threshold override (default: {nms.iouThreshold})'
    model.input_description['confidenceThreshold'] = \
        f'(optional) Confidence Threshold override (default: {nms.confidenceThreshold})'
    model.output_description['confidence'] = 'Boxes × Class confidence (see user-defined metadata "classes")'
    model.output_description['coordinates'] = 'Boxes × [x, y, width, height] (relative to data size)'
...
def run(
        ...
        imgsz=(640, 640),  # data (height, width)
    ...
    im = torch.zeros(batch_size, 3, *imgsz).to(device)  # data size(1,3,320,192) BCHW iDetection
...
def parse_opt(known=False):
    ...
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640, 640], help='data (h, w)')
